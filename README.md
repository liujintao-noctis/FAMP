...
当前应用版本为 `0.9.0`。`cmake/FampVersion.cmake` 是 CMake 和 C++ 程序使用的版本号来源；配置时会生成 `Version.h`，程序会把版本写入 Qt 应用元数据并在主窗口标题栏显示为 `FAMP 0.9.0`。发布新版本时，还需要同步更新 `vcpkg.json` 中的 `version-string`。
...
- 为选中点云调整点大小和透明度，并使用 RGB、统一颜色、局部高程 Z 或 LAS/LAZ 逐点属性渐变着色；色带支持自动数据范围和手动范围，对话框同时显示属性类型、值数和范围。在后台执行可取消的体素降采样或统计离群点去噪，仅在成功完成后原子保存为新的 PCD，原始文件不被修改。
- LAS/LAZ 逐点属性用于图层显示，项目会保存属性摘要和着色选择并在重新打开时从源文件恢复。FAMP 可在 ASCII PCD 自定义字段中无损保存 `float64`、`int64` 和 `uint64` 属性及其 Unicode 名称/单位；重投影会保持逐点对应关系并写回这些属性。预处理、裁剪和配准生成的 PCD 暂不复制属性，但不会修改原始 LAS/LAZ 文件。
...
- 支持在后台执行 ICP 刚性配准，将源点云对齐到目标点云并另存为新 PCD；可使用体素降采样加速大点云配准，无有效重叠时不会生成伪结果，原始点云不变，操作可安全取消。
- 预处理、裁剪和配准生成的 PCD 会嵌入 FAMP 双精度空间参考注释；文件仍兼容标准 PCD 工具，重新载入 FAMP 时可恢复原点和 4×4 变换。
...
- 新建、打开和原子保存 `.famp` 项目，并每 60 秒自动备份未保存更改；项目格式 v3 保存稳定图层 ID、名称、图层 CRS、锁定/可见状态、显示参数、点属性摘要、考古字段、控制点、双精度空间变换、二维图元、二维/三维测量成果、比例尺和窗口布局，兼容读取 v1/v2 项目及未包含三维测量或控制点字段的早期 v3 项目，源点云移动后可交互重新定位。
- 为项目记录经 PROJ 验证的 EPSG 坐标系，并使用通用坐标转换器核对 CRS；选择点云后可使用“点云局部/真实坐标”按保存的双精度原点和变换矩阵双向换算坐标。
- 可使用“工具 → 重投影所选点云”在后台逐点执行 CRS 转换；结果重新中心化并原子另存为带双精度空间参考的 PCD，原有逐点属性按类型和值顺序无损保留，当前图层切换到新文件。该图层原有三维测量和控制点会被移除，撤销时与图层状态一并恢复。
...
- DEM 会原子保存为版本化 `.famp-dem` 边车，可选导出 ESRI ASCII Grid、DEM CSV、等高线 CSV 和 SVG。等高线可作为可移动、可撤销的二维图元加入制图画布，会随比例尺重绘并保存到 `.famp` 项目；真实投影坐标和来源 CRS/图层仍保留在图元元数据中。
...
- 挖填方结果分别报告挖方/填方面积和体积、平衡区、缺少参考的网格以及“挖方 - 填方”净体积，并显示红/蓝/灰降采样概览图。成果原子保存为版本化 `.famp-volume`，可选导出汇总 CSV、逐格 CSV 和 SVG；后台任务可取消且不会以半成品覆盖已有文件。
...
- 点云剖面会原子保存为版本化 `.famp-profile` 边车，可选导出采样段统计 CSV、走廊原始点 CSV 和可缩放 SVG，并在程序内显示成果图。剖面分析和长文件写入都可安全取消；原始点云不被修改，已有目标文件不被半成品覆盖。
...
完整版本变更见 [`CHANGELOG.md`](CHANGELOG.md)。当前开发批次的已完成内容、未完成任务和验收标准见 [`DEVELOPMENT_STATUS.md`](DEVELOPMENT_STATUS.md)。
...
3. 选择必需的 `.famp-dem` 保存路径，并按需要勾选 ASC、DEM CSV、等高线 CSV、等高线 SVG 或“加入二维制图画布”。派生文件会显示在对话框中；已有文件在开始前会再次确认并使用原子替换。
4. 生成完成后，控制台会报告网格尺寸、米制分辨率、有效/填补单元数、等高线条数、线点数、等高距和实际成果路径。取消时不会留下正在写入的半成品；若先前阶段已经完成，程序会明确列出保留的完整成果。
...
4. 选择必需的 `.famp-volume` 边车路径，并按需导出汇总 CSV、逐格 CSV 和概览 SVG。已有目标文件会在开始前统一列出并要求确认；主边车写入失败时任务失败，可选交换文件失败时会保留已完成的主成果并集中提示。
...
4. 选择必需的 `.famp-profile` 保存路径，并按需导出采样段 CSV、走廊原始点 CSV 和 SVG。已有文件会在开始前列出并再次确认，所有成果均使用原子写入。
5. 完成后会显示剖面图及源点数、走廊点数、有效采样段和实际成果路径。蓝线是所选代表高程，灰色范围是每段最低/最高高程；没有满足最少点数的段保持断开。取消时不会留下未完成文件，已经完整写完的成果会明确列出并保留。
...
当前 GoogleTest 用例覆盖点云质心与去中心化、图层/逐点属性模型、可取消任务状态机、原始坐标、OBB、统一 PCD/LAS/LAZ/PLY/XYZ 加载服务、真实压缩 LAZ 解码、LAS 逐点属性、属性着色、ICP 配准、非 ASCII 点云路径、项目文件 v1/v2/v3 与迁移、二维场景完整往返、三维测量记录的校验与项目往返、投影坐标单位、DEM 分辨率与单元统计、小空洞填补、等高线拓扑、挖填方固定高程/参考 DEM 比较、单位换算、网格对齐、结果一致性、参数界面、大网格概览降采样与端到端边车往返、点云剖面走廊/采样统计与真实坐标变换、地形、挖填方和剖面边车/交换格式原子读写及取消保护、坐标系、撤销/重做、专业导出、考古项目报告、字符串转换和物理比例尺计算。首次配置可能需要从 GitHub 下载 GoogleTest。
