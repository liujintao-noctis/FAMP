...
- 当前 LAS/LAZ 逐点属性用于原图层显示，项目会保存属性摘要和着色选择并在重新打开时从源文件恢复。预处理、裁剪和配准生成的 PCD 暂不复制这些属性，但不会修改原始 LAS/LAZ 文件；带逐点属性的图层会阻止直接重投影，避免无提示丢失属性。
...
- 可使用“工具 → 重投影所选点云”在后台逐点执行 CRS 转换；结果重新中心化并原子另存为带双精度空间参考的 PCD，当前图层切换到新文件。该图层原有三维测量会被移除，撤销时与图层状态一并恢复。
