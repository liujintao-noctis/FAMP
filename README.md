...
当前应用版本为 `0.8.0`。`cmake/FampVersion.cmake` 是 CMake 和 C++ 程序使用的版本号来源；配置时会生成 `Version.h`，程序会把版本写入 Qt 应用元数据并在主窗口标题栏显示为 `FAMP 0.8.0`。发布新版本时，还需要同步更新 `vcpkg.json` 中的 `version-string`。
...
当前 GoogleTest 用例覆盖点云质心与去中心化、图层/逐点属性模型、可取消任务状态机、原始坐标、OBB、统一 PCD/LAS/LAZ/PLY/XYZ 加载服务、真实压缩 LAZ 解码、LAS 逐点属性、属性着色、ICP 配准、非 ASCII 点云路径、项目文件 v1/v2/v3 与迁移、二维场景完整往返、三维测量记录的校验与项目往返、投影坐标单位、DEM 分辨率与单元统计、小空洞填补、等高线拓扑、点云剖面走廊/采样统计与真实坐标变换、地形和剖面边车/交换格式原子读写及取消保护、坐标系、撤销/重做、专业导出、考古项目报告、字符串转换和物理比例尺计算。首次配置可能需要从 GitHub 下载 GoogleTest。
