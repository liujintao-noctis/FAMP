...
- 可从“文件 → 导出考古项目报告”生成 PDF 或 HTML，自动汇总项目元数据、点云清单与坐标原点，以及二维和三维的距离、面积/周长与角度测量成果。
- 新建、打开和原子保存 `.famp` 项目，并每 60 秒自动备份未保存更改；项目格式 v3 保存稳定图层 ID、名称、图层 CRS、锁定/可见状态、显示参数、点属性摘要、考古字段、双精度空间变换、二维图元、二维/三维测量成果、比例尺和窗口布局，兼容读取 v1/v2 项目及未包含三维测量字段的早期 v3 项目，源点云移动后可交互重新定位。
- 为项目记录经 PROJ 验证的 EPSG 坐标系，并使用通用坐标转换器核对 CRS；选择点云后可使用“点云局部/真实坐标”按保存的双精度原点和变换矩阵双向换算坐标。
- 可使用“工具 → 重投影所选点云”在后台逐点执行 CRS 转换；结果重新中心化并原子另存为带双精度空间参考的 PCD，原有逐点属性按类型和值顺序无损保留，当前图层切换到新文件。该图层原有三维测量会被移除，撤销时与图层状态一并恢复。
- 将 PCD/LAS/LAZ/PLY/XYZ 文件拖入主窗口，或从“文件 → 最近打开”恢复最近 8 个有效文件。
- 使用 `Ctrl+Shift+Left/Right` 将选中的二维图元绕中心每次旋转 5°。
- 使用 `Ctrl+Z` 和 `Ctrl+Shift+Z`/`Ctrl+Y` 撤销或重做最近 100 步编辑，包括二维图元新增/删除/变换/组合、三维测量的新增/清除、文字字体、制图比例尺、项目 CRS 和点云重投影。
...
当前 GoogleTest 用例覆盖点云质心与去中心化、图层/逐点属性模型、可取消任务状态机、原始坐标、OBB、统一 PCD/LAS/LAZ/PLY/XYZ 加载服务、真实压缩 LAZ 解码、LAS 逐点属性、属性着色、ICP 配准、非 ASCII 点云路径、项目文件 v1/v2/v3 与迁移、二维场景完整往返、三维测量记录的校验与项目往返、坐标系、撤销/重做、专业导出、考古项目报告、字符串转换和物理比例尺计算。首次配置可能需要从 GitHub 下载 GoogleTest。
