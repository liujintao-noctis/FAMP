...
当前应用版本为 `0.4.0`。`cmake/FampVersion.cmake` 是 CMake 和 C++ 程序使用的版本号来源；配置时会生成 `Version.h`，程序会把版本写入 Qt 应用元数据并在主窗口标题栏显示为 `FAMP 0.4.0`。发布新版本时，还需要同步更新 `vcpkg.json` 中的 `version-string`。
...
- 支持在后台执行 ICP 刚性配准，将源点云对齐到目标点云并另存为新 PCD；结果沿用目标点云坐标参考，原始点云不变，操作可安全取消。
...
当前 GoogleTest 用例覆盖点云质心与去中心化、原始坐标、OBB、统一 PCD/LAS/PLY/XYZ 加载服务、ICP 配准、非 ASCII 点云路径、项目文件 v1/v2、二维场景完整往返、坐标系、撤销/重做、专业导出、考古项目报告、字符串转换和物理比例尺计算。首次配置可能需要从 GitHub 下载 GoogleTest。
