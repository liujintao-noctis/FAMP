...
- 更新时间：2026-07-17
- 当前稳定版本：`v0.9.0`
- 当前开发分支：`feat-v090-cut-fill`
- 最近完成目标：`v0.9.0` 挖填方与体积分析
...
本节记录 v0.1.0–v0.5.1 的具体完成内容。“当前主要代码”表示这些能力在当前代码树中主要由哪些文件承载，不代表早期 tag 当时已经采用完全相同的文件组织。
...
v0.9.0 既定范围内没有未完成的功能。以下属于后续版本候选，不影响 v0.9.0 验收：
...
CI 验证策略：Linux CI 最多观察 3 分钟，超过后以本页记录的完整本地验收为依据；Windows CI 不作为当前发布阻塞条件。
