...
QJsonObject makePortableWorkspaceState(const QJsonObject& state,
                                       const QDir& projectDirectory)
{
    QJsonObject portable = state;
    QJsonArray entities = portable.value(QStringLiteral("entities")).toArray();
    for (int index = 0; index < entities.size(); ++index)
    {
        if (!entities.at(index).isObject())
            continue;
        QJsonObject entity = entities.at(index).toObject();
        const QString stored = entity.value(
            QStringLiteral("assetPath")).toString().trimmed();
        if (stored.isEmpty())
            continue;
        const QString absolute = QDir::cleanPath(QDir::fromNativeSeparators(
            QFileInfo(stored).isAbsolute()
                ? stored : projectDirectory.absoluteFilePath(stored)));
        entity.insert(
            QStringLiteral("assetPath"),
            QDir::fromNativeSeparators(
                projectDirectory.relativeFilePath(absolute)));
        entity.insert(QStringLiteral("assetAbsolutePath"), absolute);
        entities.replace(index, entity);
    }
    portable.insert(QStringLiteral("entities"), entities);
    return portable;
}

QJsonObject resolveWorkspaceAssetPaths(const QJsonObject& state,
                                       const QDir& projectDirectory)
{
    QJsonObject resolved = state;
    QJsonArray entities = resolved.value(QStringLiteral("entities")).toArray();
    for (int index = 0; index < entities.size(); ++index)
    {
        if (!entities.at(index).isObject())
            continue;
        ...
        const QString stored = QDir::fromNativeSeparators(
            entity.value(QStringLiteral("assetPath")).toString().trimmed());
        if (stored.isEmpty())
            continue;
        const QString relativeCandidate = QDir::cleanPath(
            QFileInfo(stored).isAbsolute()
                ? stored : projectDirectory.absoluteFilePath(stored));
        const QString absoluteFallback = QDir::cleanPath(
            QDir::fromNativeSeparators(entity.value(
                QStringLiteral("assetAbsolutePath")).toString().trimmed()));
        const QString selected = QFileInfo::exists(relativeCandidate)
            ? relativeCandidate
            : (!absoluteFallback.isEmpty()
                   && QFileInfo::exists(absoluteFallback)
                   ? absoluteFallback : relativeCandidate);
        entity.insert(QStringLiteral("assetPath"), selected);
        entities.replace(index, entity);
    }
    resolved.insert(QStringLiteral("entities"), entities);
    return resolved;
}
}
          ...
          QString* errorMessage)
{
    ...
    const QDir projectDirectory(
        QFileInfo(normalizedProjectPath).absolutePath());
    ...
    QJsonObject workspaceState = document.workspaceState.isEmpty()
        ? famp::workspace::emptySnapshot() : document.workspaceState;
    famp::workspace::WorkspaceSnapshot parsedWorkspace;
    QString workspaceError;
    if (!famp::workspace::deserializeSnapshot(
            workspaceState, parsedWorkspace, &workspaceError))
    {
        setError(errorMessage,
                 QStringLiteral("工作区层级无效：%1").arg(workspaceError));
        return false;
    }
    ...
    workspaceState = makePortableWorkspaceState(
        workspaceState, projectDirectory);
    ...
    root.insert(QStringLiteral("workspaceState"), workspaceState);
          ...
          QString* errorMessage)
{
    ...
    if (schemaVersion >= 4)
    {
        const QJsonValue workspaceValue = root.value(
            QStringLiteral("workspaceState"));
        famp::workspace::WorkspaceSnapshot parsedWorkspace;
        QString workspaceError;
        const QJsonObject resolvedWorkspace = workspaceValue.isObject()
            ? resolveWorkspaceAssetPaths(
                  workspaceValue.toObject(), projectDirectory)
            : QJsonObject();
        if (!workspaceValue.isObject()
            || !famp::workspace::deserializeSnapshot(
                resolvedWorkspace, parsedWorkspace, &workspaceError))
        {
            setError(errorMessage,
                     QStringLiteral("项目工作区层级无效：%1")
                         .arg(workspaceError));
            return false;
        }
    ...
    loaded.workspaceState = workspaceState;
