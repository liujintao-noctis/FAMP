...
QString normalizedItemId(const QGraphicsItem* item)
{
    if (!item)
        return {};
    const QString stored = item->data(
        famp::graphicsdoc::ItemIdDataKey).toString().trimmed().toLower();
    const QUuid id(stored);
                   ...
                   QString* errorMessage)
{
    ...
    result.insert(
        QStringLiteral("id"),
        famp::graphicsdoc::ensureItemId(const_cast<QGraphicsItem*>(item)));
    ...
    if (const auto* curve = dynamic_cast<const MyItem*>(item))
    {
        ...
        if (curve->projectionType() == XOYLine)
        {
            const QVariant sectionValue = curve->data(SectionPlaneDataKey);
            const int sectionPlane = sectionValue.isValid()
                ? sectionValue.toInt() : static_cast<int>(NONE);
            if (sectionPlane != static_cast<int>(XOZ)
                && sectionPlane != static_cast<int>(YOZ))
            {
                setError(errorMessage,
                         QStringLiteral("剖面切割线方向无效。"));
                return false;
            }
            result.insert(QStringLiteral("sectionPlane"), sectionPlane);
        }
        return true;
    }
                               ...
                               QString* errorMessage)
{
    ...
    else if (type == QStringLiteral("projection"))
    {
        ...
        if (projection == static_cast<int>(XOYLine))
        {
            const int sectionPlane = object.value(
                QStringLiteral("sectionPlane")).toInt(
            ...
            curve->setData(SectionPlaneDataKey, sectionPlane);
        }
        item = std::move(curve);
    }
    ...
    {
        const QUuid parsed(idValue.toString().trimmed());
        if (!parsed.isNull())
            id = parsed.toString(QUuid::WithoutBraces).toLower();
    }
    if (id.isEmpty())
    {
        setError(errorMessage, QStringLiteral("二维图元 ID 无效。"));
        return nullptr;
    }
    item->setData(famp::graphicsdoc::ItemIdDataKey, id);
    return item.release();
}
}
...
{
QString itemId(const QGraphicsItem* item)
{
    return normalizedItemId(item);
}

QString ensureItemId(QGraphicsItem* item)
{
    QString id = normalizedItemId(item);
    if (!item || !id.isEmpty())
        return id;
    id = QUuid::createUuid().toString(QUuid::WithoutBraces).toLower();
    item->setData(ItemIdDataKey, id);
    return id;
}
                  ...
                  QString* errorMessage)
{
    ...
    while (!pending.isEmpty())
    {
        ...
        if (id.isEmpty() || itemIds.contains(id))
        {
            qDeleteAll(candidates);
            setError(errorMessage, QStringLiteral("二维图元 ID 缺失或重复。"));
            return false;
        }
        itemIds.insert(id);
