...
{
    // 选中虚线框、线宽和抗锯齿像素都可能落在 customRect 外侧。
    // 把这些像素纳入场景失效范围，移动/旋转时旧位置才能被完整擦除。
...
void MyItem::drawCustomRect(QPainter * painter)
{
    ...
    case(XOYLine):
    {
        const QVariant sectionValue = data(SectionPlaneDataKey);
        const ProjectType sectionPlane = static_cast<ProjectType>(
            sectionValue.isValid() ? sectionValue.toInt()
                                   : static_cast<int>(NONE));
        rectPen.setColor(sectionPlane == XOZ
            ? QColor(0, 180, 220)
            : QColor(34, 139, 34));
    }
    break;
...
void MyItem::drawLine(QPainter *painter)
{
    ...
    case(XOY):
    {
        ...
        //剖面线绘制
        if (this->itemPoints.size() >= 4
            && this->itemPoints.size() < 5)
        {
    ...
    case(XOYLine):
    {
        ...
        {
            const QPointF first = itemPoints.front();
            const QPointF last = itemPoints.back();
            linePath.moveTo(first);
            linePath.lineTo(last);

            const QLineF section(first, last);
            if (section.length() > 1.0e-9)
            {
                const QPointF normal(
                    -section.dy() / section.length() * 7.0,
                    section.dx() / section.length() * 7.0);
                linePath.moveTo(first - normal);
                linePath.lineTo(first + normal);
                linePath.moveTo(last - normal);
                linePath.lineTo(last + normal);
            }
        }
        const QVariant sectionValue = data(SectionPlaneDataKey);
        const ProjectType sectionPlane = static_cast<ProjectType>(
            sectionValue.isValid() ? sectionValue.toInt()
                                   : static_cast<int>(NONE));
        linePen.setColor(sectionPlane == XOZ
            ? QColor(0, 150, 190)
            : QColor(26, 120, 26));
