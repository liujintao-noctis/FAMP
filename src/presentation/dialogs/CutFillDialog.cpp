...
bool validateOptions(const Options& options, QString* errorMessage)
{
    ...
    if (paths.sidecar.isEmpty())
    {
        setError(errorMessage, QStringLiteral("挖填方成果保存路径无效。"));
        return false;
    }
    ...
    , horizontalUnitToMetre_(horizontalUnitToMetre)
{
    ...
    saveImmediately_->setObjectName(QStringLiteral("cutFillSaveImmediately"));
    outputLayout->addRow(saveImmediately_);
    outputLayout->addRow(tr("挖填方边车"), pathRow);
    ...
    connect(saveImmediately_, &QCheckBox::toggled, this,
            [this, pathRow, browse](bool enabled) {
                ...
                exportSummaryCsv_->setEnabled(enabled);
                exportCellsCsv_->setEnabled(enabled);
    ...
    exportSummaryCsv_->setEnabled(false);
    exportCellsCsv_->setEnabled(false);
...
Options CutFillDialog::options() const
{
    ...
    result.sidecarPath = saveImmediately_->isChecked()
        ? famp::cutfillio::pathWithVolumeSuffix(sidecarPath_->text())
        : QString();
