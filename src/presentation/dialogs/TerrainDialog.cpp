...
bool validateOptions(const Options& options, QString* errorMessage)
{
    ...
    if (paths.sidecar.isEmpty())
    {
        setError(errorMessage, QStringLiteral("DEM 成果保存路径无效。"));
        return false;
    }
    ...
    , horizontalUnitToMetre_(horizontalUnitToMetre)
{
    ...
    saveImmediately_->setObjectName(QStringLiteral("terrainSaveImmediately"));
    outputLayout->addRow(saveImmediately_);
    outputLayout->addRow(tr("DEM 项目边车"), pathRow);
    ...
    connect(saveImmediately_, &QCheckBox::toggled, this,
            [this, pathRow, browse](bool enabled) {
                ...
                exportAsciiGrid_->setEnabled(enabled);
                exportGridCsv_->setEnabled(enabled);
                exportContourCsv_->setEnabled(enabled);
                exportContourSvg_->setEnabled(enabled);
    ...
    exportAsciiGrid_->setEnabled(false);
    exportGridCsv_->setEnabled(false);
    exportContourCsv_->setEnabled(false);
    exportContourSvg_->setEnabled(false);
    updateDerivedPathSummary();
}
...
Options TerrainDialog::options() const
{
    ...
    result.sidecarPath = saveImmediately_->isChecked()
        ? famp::terrainio::pathWithDemSuffix(sidecarPath_->text())
        : QString();
