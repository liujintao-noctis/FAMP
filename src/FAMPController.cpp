...
void FAMPController::initializeConnections(const Ui::MainWindowClass& ui, QStandardItemModel* model, QWidget* centerDock, QComboBox* scaleCombox)
{
    ...
    connect(model, SIGNAL(itemChanged(QStandardItem*)), m_mainWindow, SLOT(slotOn_treeItemChanged(QStandardItem*)));
    ...
    connect(m_myVTK, SIGNAL(sendActProjLineEnable(bool)), m_mainWindow, SLOT(setActProjLineEnable(bool)));
    connect(m_myVTK, SIGNAL(sendActProjXOYEnable(bool)), m_mainWindow, SLOT(setActProjXOYEnable(bool)));
    connect(m_myVTK, SIGNAL(sendActProjXOZEnable(bool)), m_mainWindow, SLOT(setActProjXOZEnable(bool)));
    connect(m_myVTK, SIGNAL(sendActProjYOZEnable(bool)), m_mainWindow, SLOT(setActProjYOZEnable(bool)));
    connect(m_myVTK, SIGNAL(sendActOverLookProjEnable(bool)), m_mainWindow, SLOT(setActOverLookProjEnable(bool)));

    // ── 投影连线 ─────────────────────────────────────────────────────

    connect(ui.actProjLine, SIGNAL(triggered()), m_graphicsView, SLOT(slotOn_actProjLine_triggered()));
    ...
    connect(m_myVTK, SIGNAL(sendProjCloud2GraphicsView(pcl::PointCloud<pcl::PointXYZRGB>::Ptr)), m_graphicsView, SLOT(getProjCloudFromVTK(pcl::PointCloud<pcl::PointXYZRGB>::Ptr)));
    ...
    connect(m_myVTK, SIGNAL(sendAABBBoxXYZMAX(float, float, float)), m_graphicsView, SLOT(getAABBMaxFromVTK(float, float, float)));
    connect(m_myVTK, SIGNAL(sendIsOverLookProj(bool)), m_graphicsView, SLOT(getisOverLookProj(bool)));
