...
void expandEditableRange(double& minimum, double& maximum)
{
    if (minimum < maximum)
    {
        // The bounds are presented in decimal spin boxes before processing.
        // Keep a small outward guard so converting the extrema to editable
        // decimal values can never exclude a point that defined the range.
        const double magnitude = std::max(
            {1.0, std::abs(minimum), std::abs(maximum)});
        const double padding = std::max(
            1.0e-8,
            magnitude * std::numeric_limits<double>::epsilon() * 8.0);
        minimum = std::nextafter(
            minimum - padding, -std::numeric_limits<double>::infinity());
        maximum = std::nextafter(
            maximum + padding, std::numeric_limits<double>::infinity());
        return;
    }
    ...
    QString* errorMessage)
{
    ...
    expandEditableRange(minimumX, maximumX);
    expandEditableRange(minimumY, maximumY);
    expandEditableRange(minimumZ, maximumZ);
    ...
    const famp::tasks::CancellationCheck& shouldCancel)
{
    ...
    for (std::size_t pointIndex = 0; pointIndex < input->size(); ++pointIndex)
    {
        ...
        if (inside == options.keepInside)
        {
            ...
            result.sourceIndices.append(static_cast<qint64>(pointIndex));
        }
    }
    if (cancel(result, shouldCancel))
        return result;
