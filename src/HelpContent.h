...
<ol>
  ...
  <li><b>坐标系与重投影：</b>使用“工具 → 项目坐标系”记录 EPSG 元数据；“坐标转换器”可用 PROJ 转换单个坐标点；选择点云后可按双精度变换和原点使用“点云局部/真实坐标”双向换算，或使用“重投影所选点云”逐点转换整云、另存 PCD。重投影会移除该图层的旧三维测量，撤销时与图层状态一并恢复。</li>
