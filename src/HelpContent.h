...
<ol>
  ...
  <li><b>打开点云：</b>使用“文件 → 打开点云”选择 PCD 或 LAS 文件，也可将文件直接拖入主窗口；读取会在后台逐个完成，期间仍可查看和操作已加载内容，并可从状态栏安全取消当前文件及剩余队列。</li>
  <li><b>继续工作：</b>“文件 → 最近打开”会保留最近 8 个可用点云文件。</li>
  <li><b>检查点云：</b>在内容列表中切换可见性，并使用六个标准视图和包围盒检查数据。</li>
  <li><b>显示与预处理：</b>在内容列表选择点云后，可从“工具”调整点大小、透明度，并切换 RGB、统一颜色或局部高程 Z 渐变（自动/手动范围）；也可在后台执行体素降采样、统计离群点去噪，或按局部 X/Y/Z 范围保留内部/外部点并另存为新 PCD。处理参数可保存/载入为 JSON 方案，成功输出会自动生成方案旁车文件，取消处理不会写入或集成结果。</li>
  <li><b>切割与投影：</b>选择点云后创建水平、竖直或任意切割面，再投影到 XOY、XOZ 或 YOZ。</li>
  <li><b>二维制图：</b>生成投影连线，设置比例尺，添加文字、指北针和制图信息。</li>
  <li><b>测量成果：</b>使用“工具 → 测量距离/测量面积/测量角度”在二维画布点击测量；距离和面积可右键或双击完成，面积同时标注周长，角度按边点—顶点—边点选择，Esc 可取消。</li>
...
<table cellspacing="8">
  ...
  <tr><td>Ctrl+Alt+D / Ctrl+Alt+A / Ctrl+Alt+G</td><td>测量二维画布距离/面积/角度</td></tr>
