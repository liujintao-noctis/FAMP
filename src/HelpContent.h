...
<h2>FAMP 快速入门</h2>
<ol>
  <li><b>管理项目：</b>使用“文件 → 新建/打开/保存项目”管理 `.famp` 点云项目；项目会保存点云原始坐标与变换、可见性、二维图元、二维/三维测量成果和窗口布局，未保存更改每 60 秒自动备份；源点云移动后，打开项目时可重新定位。</li>
  <li><b>坐标系与重投影：</b>使用“工具 → 项目坐标系”记录 EPSG 元数据；“坐标转换器”可用 PROJ 转换单个坐标点；选择点云后可按双精度变换和原点使用“点云局部/真实坐标”双向换算，或使用“重投影所选点云”逐点转换整云、另存 PCD。重投影会无损保留已有逐点属性，并移除该图层的旧三维测量；撤销时属性、坐标与测量会随图层状态一并恢复。</li>
  ...
  <li><b>考古图层记录与报告：</b>选择点云后使用“工具 → 考古图层属性”填写遗址、发掘区、探方/探沟、地层、遗迹编号、年代、日期、记录人、说明及自定义字段；修改可撤销/重做并随项目保存。使用“文件 → 导出考古项目报告”生成 PDF 或 HTML，按图层列出考古字段，并汇总项目路径、坐标参考系、比例尺、点云文件/点数/原点/可见性，以及二维/三维距离、面积/周长和角度测量成果；三维成果同时列出关联图层 ID 和 CRS。</li>
