...
<ol>
  ...
  <li><b>显示、处理与配准：</b>在内容列表选择点云后，可从“工具”调整点大小、透明度，并切换 RGB、统一颜色或局部高程 Z 渐变（自动/手动范围）；也可在后台执行体素降采样、统计离群点去噪，或按局部 X/Y/Z 范围保留内部/外部点并另存为新 PCD。处理参数可保存/载入为 JSON 方案。加载至少两个点云后，可用“点云 ICP 配准”将源点云刚性对齐到目标点云并生成新 PCD；ICP 要求初始位置足够接近。取消操作不会写入或集成结果。</li>
